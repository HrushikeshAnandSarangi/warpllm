...
pub(crate) enum Outcome {
    ...
    /// The status and raw body, verbatim, for the caller to map.
    Status {
        status: u16,
        body: String,
    },
...
pub(crate) async fn post(
    ...
    if !(200..300).contains(&status) {
        return Ok(Outcome::Status { status, body: text });
    }
...
mod tests {
    ...
    /// provider may envelope its errors differently from the dialect default.
    #[tokio::test]
