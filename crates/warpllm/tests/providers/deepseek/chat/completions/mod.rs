...
#[tokio::test]
async fn deepseek_forwards_params_for_the_provider_to_judge() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/chat/completions"))
        .respond_with(ResponseTemplate::new(200).set_body_json(openai_completion_body()))
...
#[tokio::test]
async fn deepseek_logprobs_without_refusal_decodes() {
...
#[tokio::test]
async fn deepseek_errors_name_deepseek() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
